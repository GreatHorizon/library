...
class AutocompleteEntry(Entry):
    def __init__(self, autocompleteList, callbackOnSelection=None, *args, **kwargs):
        if (callbackOnSelection is not  None):
            self.callback = callbackOnSelection
        else:
            self.callback = lambda text: None
        # Listbox length
    ...
    def selection(self, event):
        if self.listboxUp:
            ...
            self.callback(self.listbox.get(ACTIVE))
            self.listbox.destroy()
            self.listboxUp = False
            self.icursor(END)
...
if __name__ == '__main__':
    ...
    root = Tk()
    self.entry = AutocompleteEntry(autocompleteList,None, root, listboxLength=6, width=32, matchesFunction=matches)
